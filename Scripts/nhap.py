...
# GETTING THE CORNERS OF THE BOXES
def euclidean_distance(point1, point2):
    return np.sqrt((point2[0] - point1[0]) ** 2 + (point2[1] - point1[1]) ** 2)
def measure_objects(image_path, aruco_dict, parameters, pixel_cm_ratio):
    # Load the image
    ...
    # Assign points based on the box index
        if i == 0:
            ...
            dimensions[i] = {'width': width, 'height': height}
            # First box
...
# GETTING 4 POINTS
processed_img, points, dimensions = measure_objects('../Images/P2.jpg', aruco_dict, parameters, pixel_cm_ratio) #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
all_points = np.array([ point_with_smallest_X_D, point_with_smallest_X_C, point_with_smallest_X_B, point_with_smallest_X_A], dtype=np.float32).reshape(-1, 1, 2)
vx, vy, x0, y0 = cv2.fitLine(all_points, cv2.DIST_L2, 0, 0.01, 0.01)
vx, vy, x0, y0 = vx[0], vy[0], x0[0], y0[0]

###### FINE THE INTERSECTION AND DRAW THE PENPERDICULAR LINE
